                        ...
                        "files": [
                            {
                                ...
                                "target": "registers-snapshots/uk/gov/ons/sbr/${buildInfo.name}/${versionPrefix}${buildInfo.number}/"
                            },
                            { 
            ...
            steps {
                script {
                    def downloadSpec = """{
                        "files": [
                            {
                                "pattern": "registers-snapshots/uk/gov/ons/sbr/${buildInfo.name}/${versionPrefix}${buildInfo.number}/*",
                                "target": "${distDir}",
                                "flat": "true"
                            }
                        ]
                ...
                sshagent(["sbr-$ENVIRONMENT-ci-ssh-key"]) {
                    withCredentials([string(credentialsId: "prod_node", variable: 'PROD_NODE'), string(credentialsId: "edge_node", variable: 'EDGE_NODE')]) {
                        sh """
                            ...
                            ssh -o StrictHostKeyChecking=no  sbr-$ENVIRONMENT-ci@$EDGE_NODE touch $SERVICE_NAME/$SERVICE_NAME-all.jar
                            scp -r $distDir$SERVICE_NAME-${versionPrefix}${buildInfo.number}-all.jar sbr-$ENVIRONMENT-ci@$EDGE_NODE:$SERVICE_NAME/
                            echo "Successfully moved artifact [JAR] and scripts to $SERVICE_NAME/"
                            ssh -o StrictHostKeyChecking=no  sbr-$ENVIRONMENT-ci@$EDGE_NODE rm $SERVICE_NAME/$SERVICE_NAME-all.jar
                            ssh -o StrictHostKeyChecking=no  sbr-$ENVIRONMENT-ci@$EDGE_NODE "cd $SERVICE_NAME;"  mv $SERVICE_NAME-${versionPrefix}${buildInfo.number}-all.jar $SERVICE_NAME-all.jar
                            ...
                            ssh -o StrictHostKeyChecking=no  sbr-$ENVIRONMENT-ci@$EDGE_NODE hdfs dfs -put -f $SERVICE_NAME/$SERVICE_NAME-all.jar hdfs://$PROD_NODE/user/sbr-$ENVIRONMENT-ci/lib/$SERVICE_NAME/
