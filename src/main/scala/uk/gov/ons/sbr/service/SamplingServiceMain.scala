...
object SamplingServiceMain extends Stratification{
  ...
  def main(args: Array[String]): Unit = {
    ...
    args(0) match{
      case uk.gov.ons.sbr.globals.Globals.sampling => SparkSessionManager.withSpark{
                                                              ...
                                                              val processedArguments: SampleMethodArguments = new ServiceValidation(HiveUnitFrameRepository)
                                                                .parseSamplingArgs(args.toList)
      ...
      case uk.gov.ons.sbr.globals.Globals.stratification => SparkSessionManager.withSpark{
                                                              SessionLogger.log(msg ="Initiating Sampling Service")
                                                              val processedArguments: StratificationMethodArguments = new ServiceValidation(HiveUnitFrameRepository)
                                                                .parseStratificationArgs(args.toList)
  ...
  def doStratify(args: StratificationMethodArguments)(implicit sparkSession: SparkSession): Unit = {
    ...
    HiveUnitFrameRepository.saveDataFrameToTable(stratifiedFrameDf,args.hiveTable)
