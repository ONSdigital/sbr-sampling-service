                ...
                sshagent(["sbr-$ENVIRONMENT-ci-ssh-key"]) {
                    withCredentials([string(credentialsId: "prod_node", variable: 'PROD_NODE'), string(credentialsId: "edge_node", variable: 'EDGE_NODE')]) {
                        sh """
                            ...
                            ssh -o StrictHostKeyChecking=no  sbr-$ENVIRONMENT-ci@$EDGE_NODE rm $SERVICE_NAME/*
                            ...
                            ssh -o StrictHostKeyChecking=no  sbr-$ENVIRONMENT-ci@$EDGE_NODE hdfs dfs -test -e hdfs://$PROD_NODE/user/sbr-$ENVIRONMENT-ci/lib/$SERVICE_NAME/sbr-sampling-service-all.jar; result=$?
