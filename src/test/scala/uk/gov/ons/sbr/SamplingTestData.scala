...
trait SamplingTestData {
  ...
  val  data = List(
    ...
    ("1100000099", "9906000299", "BLAH PLC"          ,  ""  , "1 High St"          , "CITY"       , "BLX",       ""                 ,  ""  , "BLAH BLAH", "1", "46120", "2" , "2" , "287", "287", "0", "0", "0", "0.165647458", "5813"),
  ...
  val props = List(
    //("687","5813","Census","P","45111","45190","100","999999999","0.129110904","4"),
    ("687","5813","Census","P","45111","45190","100","999999999","0.129110904","4"),
  ...
  def dataDF(implicit spark:SparkSession) = {
    val dataDF = spark.createDataFrame(data).toDF(dataColNames: _*)
    dataDF
  }

  def propsDF(implicit spark:SparkSession) = {
